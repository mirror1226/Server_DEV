...
public class ThreadServer implements Runnable{
	...
	/*
	 * 构造函数
	 * 参数：与客户端相连的Socket对象socket，线程好pid
	 */
...
//			System.out.println("ClientIP:" + ClientIP);
	...
	private int ParseInstruction (String instruction)
	{
		...
		//空指令
		if (instruction.equals("")) return 0;
	...
	/*
	 * 控制指令解析
	 * 参数：指令类型号type，指令内容content
	 * 返回值：指令正确执行则返回1，否则返回0
	 */
	private int ParseCtlInstruction (String type, String content)
	{
...
//		System.out.println ("function:" + function);
	 ...
	 * 返回值：执行成功返回1，否则返回0
	 */
	private int ExecGameCtl (String type, String content)
	{
...
//		System.out.println("content:" + content);
	 ...
	 * 返回值：执行成功返回1，否则返回0
	 */
	private int ExecPlayerCtl (String type, String content)
	{	
		//解析指令型号
	...
	{		
		...
		case "01" :
			...
			PlayerInfo playerInfo = new PlayerInfo (playerName, password, phone, Email);			
		...
		case "03" :
			...
			PlayerOnlineInfo playerOnlineInfo = new PlayerOnlineInfo (playerName, IPAdr);
	 ...
	 * 返回值：执行成功返回1，否则返回0
	 */
	private int ExecPlayerCtlFriend (String type, String content)
	{
		//解析指令内容
		String fields[] = content.split("\\$");
		String gameName = fields[0];
		...
		case "11" :
			...
			return playerOperate.RepForFriend(gameName, playerNameReq, playerNameRec);
		...
		case "12" :
			...
			return playerOperate.AckForFriend(gameName, playerNameReq, playerNameRec);
	 ...
	 * 返回值：执行成功返回1，否则返回0
	 */
	private int ExecPlayerCtlLocation (String type, String content)
	{
		//解析指令内容
		String fields[] = content.split("\\$");
		String gameName = fields[0];
