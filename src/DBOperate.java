...
public class DBOperate {
	...
	private java.sql.Connection ConDB ()
	{
		try {
			java.sql.Connection conn = java.sql.DriverManager
					.getConnection (strConDB, rootUserName, rootPassword);
			if (conn != null) {
				return conn;
			} else {
				return null;
	 ...
	 * 查找周围的在线玩家
	 */
	public String selectPlayerNearby(String gameName, double latitude, double longtitude)
	{
		String query = "select username, latitude, longtitude from " + gameName + "_Online";
		java.sql.ResultSet rs = DBQuery(query);
		...
		try {
			while (rs.next()){
				...
				if (distance(latitude, longtitude, rs_lat, rs_lng) <= 1)	//在方圆1公里范围内
	 ...
	 * 根据关键词匹配用户名	
	 */
	public String selectByID(String gameName, String playerName, String key) {
		String query = "select username from " + gameName + "_Info where username like '%" + key + "%';";
		java.sql.ResultSet rs = DBQuery(query);
		String result = "Result";
		try {
			while (rs.next()) {
	 ...
	 * 查找在线好友
	 */
	public String selectFriendsOnline(String gameName, String playerName) {
		String query = "select name from " + gameName + "_Online, " + gameName + "_" + playerName + "_Friend " + " where username = name;";
		java.sql.ResultSet rs = DBQuery(query);
		String result = "Result";
		try {
			while (rs.next()) {
	 ...
	 * 返回playerName的相关消息
	 */
	public String selectMessage(String gameName, String playerName) {
		String query = "select type, date, srcName, Content from " + gameName + "_" + playerName + "_Message";
		java.sql.ResultSet rs = DBQuery(query);
		String result = "Result";
		try {
			while (rs.next()) {
				result = result + "," + rs.getString("type") + ":" + rs.getString("date") + ":" + rs.getString("srcName") + ":" + rs.getString("Content");
			}
			return result;
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public int isFriend(String gameName, String playerName, String friend) {
		String query = "select name from " + gameName + "_" + playerName + "_Friend where name='" + friend + "';";
		java.sql.ResultSet rs = DBQuery(query);
	...
	public String selectFriendInfo(String gameName, String friend) {
		String query = "select username, Phone, Email from " + gameName + "_Info where username='" + friend + "';";
		java.sql.ResultSet rs = DBQuery(query);
		
		try {
			if (rs.next()) {
				String result = "Result:" + rs.getString("username") + " " + rs.getString("Phone") + " " + rs.getString("Email");
				return result;
			}
			else return "0";
		} catch (SQLException e) {
	...
	private double distance(double latA, double lngA, double latB, double lngB){	//计算两个经纬度之间的距离
